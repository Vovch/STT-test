...
- use **Hold to Ask Web** in the main window to record a spoken query, then release to transcribe and run your **web search command** (built-in **OpenCode** when the command field is empty). When the Potato STT window is visible, the summary opens in a small in-app window; when the app is minimized or hidden to the tray, the summary is announced via the **system tray balloon** instead (click the balloon to open the same summary window). Under **Options**, set a custom command line with placeholders `{query}`, `{prompt}` (OpenCode-style instruction block), and `{query_json}`; optional **system shell** mode is available for pipes/redirection. You can bind the same kinds of keys, mouse buttons, or keyboard chords as push-to-talk for **Hold to Ask Web** (optional). **Multi-tap on push-to-talk keys** (optional) reuses the same physical key as dictation: either **tap once, release, then press-and-hold** for web while a plain **hold** still dictates after a short delay, or **double-tap-then-hold** for dictation and a **third tap** to start a web query. The multi-tap delay (ms) controls both the hold threshold and the time between taps. An on-screen cue shows while recording and while the search runs.
...
5. For web-style answers, press and hold **Hold to Ask Web**, speak your query, and release. Or enable the double-tap option: hold your push-to-talk key/button once for normal dictation, or tap it once then press-and-hold it again to record a web query. Adjust the double-tap delay in Options if the second press is too easy or too hard to trigger. The app runs `opencode` in non-interactive mode and shows the returned summary in a popup.
6. Expected:
...
- **Tray balloons:** Windows may ignore the requested display duration or click behavior for `QSystemTrayIcon` notifications depending on shell settings; click-to-open still works when the OS delivers `messageClicked`. The balloon **body** is a short plain excerpt: the shell caps tray text at roughly **256 UTF-16 code units** (Potato STT uses ~**248** on Windows so the excerpt fits reliably; click the balloon for the complete answer). Optional: `POTATO_STT_WEB_TRAY_BODY_MAX_CHARS` (integer, clamped) to lower that budget if you prefer shorter balloons.
